...
export const votePost = async (postData: any, currentUser: any) => {
  ...
  try {
    const { query: getVoteSql, values: getVoteValues } =
      postQuery.getVote(postId, userId);
    ...
    const { query: incrSql, values: incrValues } =
      postQuery.incrementVote(postId, voteIncrement);

    const updated = await query(incrSql, incrValues);

    const authorId =
      updated.rows[0].studentAuthorId ||
      updated.rows[0].orgAuthorId;

    if (authorId && authorId !== userId) {
      const { query: repSql, values: repValues } =
        usersQuery.recalculateReputation(authorId);
