...
export const votePost = async (postData: any, currentUser: any) => {
  ...
  try {
    const { query: getVoteSql, values: getVoteValues } = postQuery.getVote(
      postId,
      userId,
    );
    ...
    if (existingVote === type) {
      ...
      repIncrement = type === "up" ? 12 : -12;
      userVote = type;
    }

    const { query: incrSql, values: incrValues } = postQuery.incrementVote(
      postId,
      voteIncrement,
    );
    const updated = await query(incrSql, incrValues);
    console.log("rows:", updated.rows, "rowCount:", updated.rowCount);
    console.log("updated post row:", updated.rows[0]);
    console.log("authorId:", updated.rows[0].studentAuthorId);
    console.log("userId (voter):", userId);
    console.log("repIncrement:", repIncrement);

    const authorId = updated.rows[0].studentAuthorId;
    if (authorId && authorId !== userId) {
      const { query: repSql, values: repValues } =
        usersQuery.incrementReputation(authorId, repIncrement);
      await query(repSql, repValues);
    }

    return { success: true, post: { ...updated.rows[0], userVote } };
  } catch (err) {
