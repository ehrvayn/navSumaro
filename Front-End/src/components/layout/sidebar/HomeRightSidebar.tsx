...
const RightSidebar: React.FC<RightSidebarProps> = ({ isOpen, onClose }) => {
  ...
  const content = (
    <div className="pt-5 mt-[60px] pr-2 md:mt-0">
          ...
          {events.length > 0 ? (
            ...
            <div className="space-y-3">
              {[...Array(5)].map((_, i) => (
                <div
                  key={i}
                  className="bg-base-surface border border-border rounded-md p-3 flex items-center gap-3 animate-pulse"
                >
                  <div className="min-w-[42px] h-[42px] rounded-md bg-gray-200 flex-shrink-0" />
                  <div className="min-w-0 flex-1">
                    <div className="h-3 bg-gray-300 rounded-sm w-3/4 mb-2" />
      ...
      <section className="mb-6">
        <SectionHeader
          title="Trending Topics (mock)"
          action="See all →"
          onAction={() => alert("This feature is under Development!")}
        />
