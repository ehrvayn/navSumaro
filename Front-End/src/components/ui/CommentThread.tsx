...
const countAllReplies = (commentId: string, allComments: Comment[]): number => {
  ...
  return direct.reduce(
    (acc, r) => acc + 1 + countAllReplies(r.id, allComments),
    0,
  );
};
...
const CommentThread: React.FC<CommentThreadProps> = ({
  ...
  const { setSelectedPostId, setPostUserProfileId, getUserData } = usePosts();
      ...
      <div className="flex gap-1 min-w-0">
        <div className="relative flex flex-col items-center shrink-0 w-8">
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              setPostUserProfileId(c.author.id);
              setActivePage("profile");
              getUserData(c.author.id);
        ...
        <div className="flex-1 pb-3 min-w-0">
          <div className="bg-base-surface border border-border rounded-md p-3 shadow-sm hover:border-border-accent transition-colors">
            <div className="flex justify-between items-start mb-1 gap-2">
              ...
              <span className="text-[10px] sm:text-[10px] text-text-muted font-medium shrink-0">
                {timeAgo}
              </span>
            </div>
            ...
            {isLong && showFull && (
              <button
                ...
                className="text-brand text-[10px] sm:text-xs font-semibold mb-1 hover:text-orange-400 transition-colors block"
              >
                Show less
              </button>
            )}
            <p
              className="text-[11px] sm:text-[12px] md:text-[13px] text-text-secondary leading-relaxed font-sora break-words"
              style={
                !showFull && isLong
                  ? {
                      display: "-webkit-box",
            ...
            {isLong && (
              <button
                ...
                className="text-brand text-[10px] sm:text-xs font-semibold mt-1 hover:text-orange-400 transition-colors"
              >
          ...
          <div className="flex items-center justify-between pr-4 gap-3 mt-1.5 ml-1">
            <div className="flex px-4 gap-3">
              <button
                onClick={() => onLike(c.id)}
                className={`flex items-center gap-1 text-[9px] sm:text-[10px] font-bold active:scale-75 transition-colors ${
                  c.liked ? "text-brand" : "text-text-muted hover:text-brand"
                }`}
              >
                <Heart
                  size={14}
              ...
              <button
                className="text-[9px] sm:text-[10px] font-bold text-text-muted hover:text-text-primary transition-colors"
                onClick={() => setActiveReply((v) => !v)}
              >
                REPLY
              </button>
            </div>
            {totalReplies > 0 ? (
              <button
                className="text-[9px] sm:text-[10px] flex items-center gap-0.5 font-bold text-text-muted hover:text-text-primary transition-colors"
                onClick={() => setViewReplies((v) => !v)}
              >
                {viewReplies
                ...
                <ChevronDown
                  size={13}
                  className={`sm:w-[15px] sm:h-[15px] transition-transform duration-200 ${viewReplies ? "rotate-180" : ""}`}
                />
              </button>
            ) : (
              <span />
          ...
          {replies.map((r, i) => (
            <div key={r.id} className="flex min-w-0">
              <div className="relative shrink-0 w-8">
                {i < replies.length - 1 && (
                  <div className="absolute top-0 bottom-0 left-1/2 -translate-x-1/2 w-[2px] bg-gray-700" />
                )}
                <div
                  className="absolute left-1/2 -translate-x-[1px] w-6 h-5 border-l-2 border-b-2 border-gray-700 rounded-bl-lg"
                  style={{ top: 0 }}
                />
              </div>
